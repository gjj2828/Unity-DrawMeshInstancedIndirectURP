Shader "Instanced/InstancedShader_URP" {
    Properties
    {
    ...
    SubShader{
        ...
        Pass {
            ...
            #else
                float4 data = float4(0.0, 100.0, 0.0, 1.0);
            #endif
            ...
            {
               half4 albedo = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, IN.uv) * IN.color;
        ...
        Pass {
           ...
           Vary Vertex(Attr IN, uint instanceID : SV_InstanceID)
           {
               Vary OUT;
           #if SHADER_TARGET >= 45
               float4 data = positionBuffer[instanceID];
           ...
           half4 Fragment(Vary IN) : SV_TARGET
           {
               return 0;
           }
